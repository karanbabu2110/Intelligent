...
final class KnowledgeRetrieveCommand {
    ...
    int execute(String queryText) {
        PreparationOutcome outcome = prepare(queryText);
        if (!outcome.successful()) return outcome.exitCode();
        GroundedPrompt groundedPrompt = outcome.prompt();
        List<RetrievedContext> matches = outcome.matches();
        ...
        printCitations(groundedPrompt.citations());
        return KaosApplication.SUCCESS;
    }

    PreparationOutcome prepare(String queryText) {
        try {
            ...
            if (!embedding.successful()) {
                return PreparationOutcome.failed(reportEmbeddingFailure(embedding.status()));
            }
    ...
    void printCitations(List<SourceCitation> citations) {
    ...
    record PreparationOutcome(
            int exitCode, GroundedPrompt prompt, List<RetrievedContext> matches) {
        PreparationOutcome {
            matches = List.copyOf(matches);
        }

        boolean successful() {
            return exitCode == KaosApplication.SUCCESS;
        }

        private static PreparationOutcome success(
                GroundedPrompt prompt, List<RetrievedContext> matches) {
            return new PreparationOutcome(KaosApplication.SUCCESS,
                    Objects.requireNonNull(prompt, "prompt"), matches);
        }

        private static PreparationOutcome failed(int exitCode) {
            return new PreparationOutcome(exitCode, null, List.of());
        }
    }
}
